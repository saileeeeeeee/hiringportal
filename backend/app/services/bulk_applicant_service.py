...
# Reuse existing helpers (make sure these are in the file or imported)
#from .applicant_service import save_resume, trigger_evaluate_resume_match, get_jd
# For completeness, assuming they exist
...
def create_applicant_from_pdf(
    ...
    try:
        ...
        temp_resume_path = save_resume(pdf_file, applicant_id=None)
        ...
        with db.begin():
            ...
            res = db.execute(text("SELECT SCOPE_IDENTITY() AS id"))
            applicant_id = res.fetchone()[0]

            final_path = os.path.join(
                os.path.dirname(temp_resume_path),
                f"{applicant_id}_{pdf_file.filename}"
            )
            os.rename(temp_resume_path, final_path)
            ...
            db.execute(text("""
                INSERT INTO applications (
                    ...
                    :applicant_id, :job_id, :application_status, :source,
                    :assigned_hr, :assigned_manager, :comments,
